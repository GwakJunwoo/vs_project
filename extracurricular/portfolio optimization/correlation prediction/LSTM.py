...
train_X= pd.read_csv('after_arima/train_X.csv')
print('loaded train_X')
dev_X = pd.read_csv('after_arima/dev_X.csv')
print('loaded dev_X')
test1_X = pd.read_csv('after_arima/test1_X.csv')
print('loaded test1_X')
test2_X = pd.read_csv('after_arima/test2_X.csv')
print('loaded test2_X')
train_Y = pd.read_csv('after_arima/train_Y.csv')
print('loaded train_Y')
dev_Y = pd.read_csv('after_arima/dev_Y.csv')
print('loaded dev_Y')
test1_Y = pd.read_csv('after_arima/test1_Y.csv')
print('loaded test1_Y')
test2_Y = pd.read_csv('after_arima/test2_Y.csv')
...
_train_X = np.asarray(train_X).reshape((int(2800/STEP), 20, 1))
_dev_X = np.asarray(dev_X).reshape((int(2800/STEP), 20, 1))
_test1_X = np.asarray(test1_X).reshape((int(2800/STEP), 20, 1))
_test2_X = np.asarray(test2_X).reshape((int(2800/STEP), 20, 1))

_train_Y = np.asarray(train_Y).reshape(int(2800/STEP), 1)
_dev_Y = np.asarray(dev_Y).reshape(int(2800/STEP), 1)
_test1_Y = np.asarray(test1_Y).reshape(int(2800/STEP), 1)
_test2_Y = np.asarray(test2_Y).reshape(int(2800/STEP), 1)
...
for _ in range(124):
    ...
    if len(file_list) != 0 :
        ...
        recent_model_name = 'r_epoch'+str(epoch_num-1)+'.h5'
        filepath = 'models/' + d + '/' + recent_model_name
        model = load_model(filepath)

    filepath = 'models/' + d + '/r_epoch'+str(epoch_num)+'.h5'
