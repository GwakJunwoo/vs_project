...
def get_assets(ticker_count):
    bundle = bundles.load('FBA_bundle')
...
def test_mean_reversion_5day_sector_neutral(fn):
    ...
    start_date_str = '2016-01-07'
    end_date_str = '2016-01-11'
    ...
    run_pipeline_args = {
        'pipeline': pipeline,
        'start_date': pd.Timestamp('2016-01-07', tz='utc'),
...
def test_mean_reversion_5day_sector_neutral_smoothed(fn):
    ...
    run_pipeline_args = {
        'pipeline': pipeline,
        'start_date': pd.Timestamp('2016-01-14', tz='utc'),
        'end_date': pd.Timestamp('2016-01-19', tz='utc')}
    fn_inputs = {
        'window_length': 3,
        'universe': universe,
        'sector': project_helper.Sector()}
...
def test_optimal_holdings_get_constraints(cl):
    ...
    def solve_problem(weights):
        ...
        risk1 = cvx.quad_form(factor_betas.T @ weights, x)
        risk2 = cvx.quad_form(weights, s)
        risk = risk1 + risk2
        constaints = optimal_holdings._get_constraints(weights, factor_betas, risk)
        obj = cvx.Maximize(weights.T @ [0, 1, 5, -1])
        prob = cvx.Problem(obj, constaints)
        prob.solve(max_iters=500)
